...
namespace NAudio.Wave 
{
    ...
    public class WaveOut : IWavePlayer, IWavePosition
    {
        ...
        public void Init(IWaveProvider waveProvider)
        {
            ...
            lock (waveOutLock)
            {
               result = callbackInfo.WaveOutOpen(out hWaveOut, DeviceNumber, waveStreamReader.WaveProvider.WaveFormat, callback);
            }
            ...
            {
               // TODO:
               buffers[n] = new WaveOutBuffer(hWaveOut, new byte[bufferSize], waveOutLock);
            }
        }        
        ...
        private void EnqueueBuffers()
        {
            for (int n = 0; n < NumberOfBuffers; n++)
            {
                ...
                {
                   byte[] buffer;
                   if (waveStreamReader.Read(out buffer))
                   {
                      buffers[n].OnDone();
